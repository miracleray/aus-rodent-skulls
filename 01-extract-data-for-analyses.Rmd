...
## Load the data 
```{r}
my_data <- read.csv("data/18_07_20_7Roundsminus20.csv", header = T)
```
Note that the data is loaded in using a relative directory - not a setwd command. This allows others who have cloned this repo of code and data to still use the code out of the box. 
...
Our methodology tested whether different patch protocols impact later shape analyses. To save time, we digitized the patches at the same time so we have two different patch protocols to extract from the same shape coordinate file. 
