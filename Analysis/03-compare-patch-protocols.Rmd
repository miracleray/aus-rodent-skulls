...
Here we use Dr Thomas Guillerme's new package, `landvR` to see how landmarks vary within and between the two protocols. Much of the below is adapted from [this vignette written by Dr Guillerme](https://cdn.rawgit.com/TGuillerme/landvR/8a6a6bd5/inst/vignettes/Landmark_partition_test.html).
```{r}
...
PC1.var.plot <- procrustes.var.plot(big.hypothetical.3, big.hypothetical.4, col = heat.colors, col.val = big.PC1.var[, "radius"], labels = F)
```
...
# Plot the range of variation along PC2 using a heat color scheme
PC1.var.plot <- procrustes.var.plot(sm.hypothetical.3, sm.hypothetical.4, col = heat.colors, col.val = sm.PC1.var[, "radius"], labels = F)
```
...
# Colors by EGenus: Hyd Leg Lep Mel Mes Mus Not Pog Pse(+Mas) Rat Uro Xer Zyz
col.gen <- c("red", "cornflowerblue", "light green", "yellow", "green", "grey", "blue", "magenta", "dark blue", "black", "darkgoldenrod4", "orange", "light blue")
...
col.phylo <- c("yellow", "darkgoldenrod4", "green", "light green", "cornflowerblue", "light blue", "blue", "dark blue", "red", "orange", "magenta", "grey", "black")  # same as col.gen but in a different order
gen.name.phylo <- c("Mel", "Uro", "Mes", "Lep", "Leg", "Zyz", "Not", "Pse", "Hyd", "Xer", "Pog", "Mus", "Rat")  # order of genera in phylogeny
```
...
plot(x = big.pca$pc.scores[, 1],
     ...
     cex.axis = 1.3, 
     cex.lab = 1.3)
legend(0.15, 0.055, legend = gen.name.phylo, col = col.phylo, pch = 16, cex = 0.78)
```

### PCA plot of small patch protocol
```{r}
# Plot PCAs
sm.pca <- plotTangentSpace(sm.shape, groups = sp.col.gen, axis1 = 2, axis2 = 1, verbose = T)
...
Big patch protocol:
```{r}
gdf <- geomorph.data.frame(genus = big.info$EGenus, size = big.info$Csize)
