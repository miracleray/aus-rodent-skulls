...
coords.3D <- coords.3D[1:(length(coords.3D) - 40)]  # removes 1st 20 specimens landmarked, with 2 protocols per specimen = 40
```
...
We correct the naming scheme to be machine-readable and in the process create a list of specimen names which were landmarked twice. In script 02-calculate-user-error.Rmd, we use this list to analyze user error relative to variation in our sample. 
```{r}
# Detect replicates
names <- dimnames(A)[[3]]
...
### Split shape dataset into big and small patch datasets
```{r}
...
1. For bilateral names, "R" and "L" designate sides after the landmark's name.
...
```{r}
# Write landmark pair tables for each protocol
...
Here, we simply join the centroid size generated by `gpagen()` to the metadata tables for each dataset. These data will be used later in the script to test allometry.

WARNING: the `bilat.symmetry()` function will change the order of specimens IF you have any non-unique names in the vector given to argument `ind =` of `bilat.symmetry()`. For example, if we used `$CatNum`, then we would have 2 of the same CatNums for specimens with replicates. To avoid this, have a column for Replicate Status ("Rep" in this script) and use the `$All` column for the `ind =` argument of `bilat.symmetry()`.
```{r}
...
```{r}
save(big.bY.shape, big.sp.info, rep.spec, file = "../Data/Processed/01-big-data.rda")
save(sm.bY.shape, sm.sp.info, file = "../Data/Processed/01-sm-data.rda")
```