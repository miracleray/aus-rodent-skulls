...
'Give_LMs()' returns a slider matrix with corrected landmark numbers for one curve - given by row number, i - at a time. 
```{r}
...
* the user-generated character matrix - ours came from curve_LM_key.csv - with the column listing unique curve names called "Curve"
* a 3-column slider matrix with columns Before, Slider, and After in that order, initiated by 'Dummy_Slider()' above.

'Write_Slider()' returns a completed slider matrix.
```{r}
...
save(big_Y, sm_Y, file = "../Data/Results/02-GPA-data.rda")
```