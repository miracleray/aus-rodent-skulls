...
### Write function to identify invasive specimens
The output of this function will allow us to subset both the info and shape datasets to include only native species. 
```{r}
...
Allo <- procD.allometry(coords ~ csize, ~ clade, logsz = TRUE, iter = 999, RRPP = TRUE, print.progress = FALSE, data = gdf)
summary(Allo)
...
plot(x = log(Allo$size),
     ...
     xlab = "Log centroid size", 
     ylab = "Predicted shape")
legend(6.04, 0.108, legend = clade.names, col = col.clade.legend, pch = 16, cex = 0.78)
```