...
cor.assoc <- round(unname(cor$estimate), digits = 2) # 2 sig figs

## Set color scheme for legend, taxa listed in order presented in phylogeny
gen_color <- c("yellow", "darkgoldenrod4", "green", "light green", "cornflowerblue", "light blue", "blue", "dark blue", "red", "orange", "magenta", "grey", "black")
genus_name <- c("Mel", "Uro", "Mes", "Lep", "Leg", "Zyz", "Not", "Pse", "Hyd", "Xer", "Pog", "Mus", "Rat")

## Plot centroid size versus PC1 to see correlation
plot(info$Csize, pca$pc.scores[,1], xlim = c(120, 440), ylim = c(-0.13, 0.13), col = col.gen, pch = 16, xlab = "Centroid size", ylab = "PC1 score", main = "PC1 vs Centroid Size")
legend(413, 0.15, legend = genus_name, col = gen_color, pch = 16, cex = 0.78)
...
## Plot results
plot(log(Allo$size), (Allo$pred.val), xlim = c(4.72, 6.17), col = col.gen, pch = 16, xlab = "Log centroid size", ylab = "Predicted shape", main = "Predicted Allometric Shape by Genus")
legend(6.04, 0.108, legend = genus_name, col = gen_color, pch = 16, cex = 0.78)
```