...
```{r message = FALSE}
library(devtools)  # needed only to install landvR from Github the 1st time
...
# Stores mean shapes in 2D array recognized by geomorph (n x [p x k])
mean.shapes <- NULL  # initiate blank data frame
for (i in 1:length(full.names)) {
        is.taxa <- which(info.traits$FullName == full.names[i])
        shape.for.mean <- shape[, , is.taxa]
        mean.of.taxa <- mshape(shape.for.mean)
        mean.shapes <- cbind(mean.shapes, mean.of.taxa)
}
```
