...
legend(0.142, 0.047, legend = info.means$Taxa, col = col.means, border = NULL, pch = pch.means, pt.bg = col.means, cex = 0.305)
```
...
We created a function, `MatchedTips()` to do most of the work (see utilities.R).
```{r}
...
MatchTips(aus.tree, unique(info.traits$FullName))

info.tree <- info.traits[MatchTips(aus.tree$tip.label, info.means$FullName)$matched, ]

shape.tree <- shape[ , , MatchTips(aus.tree$tip.label, dimnames(mean.shapes)[[3]])$matched]
```
