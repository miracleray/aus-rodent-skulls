...
for (i in 1:length(full.names)) {
        ...
        mean.shapes.2D <- rbind(mean.shapes.2D, mshape(shape.for.mean))  # mshape() finds mean specimen; rbind() is critical, output cols must = 3 for 3D
}
mean.shapes <- arrayspecs(mean.shapes.2D, 325, 3)  # convert to 3D array
```

Here, we manage the names and metadata for shape to ensure they match the tree names (Genus_species) and share the shape data's order (alphabetical by names). 
```{r}
...
PlotPCA(pca.means, 1, 2, color.means, pch.means, flip.axis1 = TRUE)
```
