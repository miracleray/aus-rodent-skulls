...
### Write function to subset specimens by region
Here, we subset the PCA scores by species which inhabit the 7 different ecological regions of Australia. Regions and species distributions therein taken from Breed & Ford 2006.
```{r}
## Add argument to FoundInRegion re: include invasives? 
...
Write function to give plotting points (pch values) by unique species within each genus.
```{r}
...
Plot PCAs with color by taxa for each region
```{r}
...
for (i in 1:7) {
        ...
        PCA.plot <- plot(x = pc.region$PC1,
                         ...
                         cex.axis = 1.3, 
                         cex.lab = 1.3)
        legend(0.06, 0.08, legend = unique.taxa, col = col.gen.legend, border = NULL, pch = pch.tax.legend, pt.bg = col.gen.legend, cex = 0.64)
}
```
